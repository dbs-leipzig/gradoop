...
public class TemporalElementStats implements Serializable {
     ...
     */
    private double[] txFromValFromStats;
     ...
     * Samples vertices
     */
    private ReservoirSampler<TemporalElement> sampler;
    ...
    private void computePropertyEstimations(){
        ...
        // classifiy and collect all property values
    ...
    private void computeCategoricalEstimations(Map<String, List<PropertyValue>> data,
        ...
        for(String key : data.keySet()){
            ...
            for(PropertyValue value: data.get(key)){
                selectivityMap.put(value,
                        selectivityMap.getOrDefault(value, 0.)+1./sampleSize);
            }

            categoricalSelectivityEstimation.put(key, selectivityMap);
        }
    ...
    private void computeNumericalEstimations(Map<String, List<PropertyValue>> data,
        ...
        numericalPropertyStatsEstimation = new HashMap<>();
        for(String key: data.keySet()){
            List<PropertyValue> values = data.get(key);

            numericalOccurrenceEstimation.put(key,
            ...
            numericalPropertyStatsEstimation.put(key, new Double[]{mean, variance});
     ...
     * its occurence (based on the current reservoir sample)
