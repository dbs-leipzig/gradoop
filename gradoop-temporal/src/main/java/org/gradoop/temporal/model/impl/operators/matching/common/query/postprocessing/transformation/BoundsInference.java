...
public class BoundsInference implements QueryTransformation {
  ...
  public CNF transformCNF(CNF cnf) throws QueryContradictoryException {
    ...
    System.out.println("relevant Comparisons: "+relevantComparisons);
    ...
    System.out.println("----initial lower and upper: ");
    System.out.println("lower: "+lowerBounds);
    System.out.println("upper: "+ upperBounds);
    System.out.println("----c-Relations-----");
    System.out.println("<=: "+cLeq);
    System.out.println("<: "+cLt);
    System.out.println("=: "+cEq);
    System.out.println("!=: "+cNeq);
    ...
    System.out.println("= : "+closures.get(0));
    System.out.println("<= : "+closures.get(1));
    System.out.println("< : "+closures.get(2));
    ...
    System.out.println("other: "+otherClauses);
    allClauses.addAll(remainingOldClauses);
    System.out.println("remaining old: "+remainingOldClauses);
  ...
  private List<ComparisonExpression> comparisons(HashMap<TimeSelector, Long> lowerBounds,
    ...
    for(TimeSelector selector : lowerBounds.keySet()) {
      Long lower = lowerBounds.get(selector);
  ...
  private List<ComparisonExpression> filterRedundantComparisons(List<ComparisonExpression> comparisons) {
    ...
    for(ComparisonExpression comp: comparisons){
      System.out.println("checking "+comp+" for redundancy ");
      ...
      if(rhs instanceof TimeSelectorComparable && lhs instanceof TimeSelectorComparable && comparator.equals(Comparator.LTE)){
        if(((TimeSelectorComparable) rhs).getVariable().equals(((TimeSelectorComparable) lhs).getVariable())){
          if(((((TimeSelectorComparable) lhs).getTimeField().equals(VAL_FROM) && ((TimeSelectorComparable) rhs).getTimeField().equals(
            VAL_TO)) || (((TimeSelectorComparable) lhs).getTimeField().equals(
            TX_FROM) && ((TimeSelectorComparable) rhs).getTimeField().equals(
            TX_TO)))){
            continue;
          }
        } else{
        ...
        // comparisons between literals not informative. Contradictory comparisons have been filtered out before
        ...
        // comparisons between selectors and literals are redundant, as they are contained in the inferred bounds
        continue;
      } else {
  ...
  private List<HashMap<TimeSelector, Long>> updateLt(
    ...
    System.out.println("LT-tuples: "+lt);
  ...
  private List<HashMap<TimeSelector, Long>> updateLeq(
    ...
    System.out.println("rLeq: "+rLeq);
    System.out.println("Relation as tuples: "+ leq);
   ...
   */
  private ArrayList<ArrayList<ComparableExpression>> relationToTuples(HashMap<ComparableExpression, HashSet<ComparableExpression>> rel){
  ...
  private List<HashMap<ComparableExpression, HashSet<ComparableExpression>>> computeClosureRelations(
    ...
    // init relations
    HashMap<ComparableExpression, HashSet<ComparableExpression>> rEq = new HashMap<>();
    HashMap<ComparableExpression, HashSet<ComparableExpression>> rLeq = new HashMap<>();
    HashMap<ComparableExpression, HashSet<ComparableExpression>> rLt = new HashMap<>();
    // compute them
