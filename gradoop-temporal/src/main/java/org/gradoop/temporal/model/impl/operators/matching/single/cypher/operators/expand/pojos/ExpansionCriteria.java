...
public class ExpansionCriteria implements Serializable {
   ...
   *                                       to impose no restriction on the value.
   * @param expandEdgeTxFromCondition      conditions for successive edges' tx_from values. {@code null}
   *                                       to impose no restriction on the value.
   * @param expandEdgeTxToCondition        conditions for successive edges' tx_to values. {@code null}
   *                                       to impose no restriction on the value.
   * @param expandEdgeValidFromCondition   conditions for successive edges' valid_from values. {@code null}
   *                                       to impose no restriction on the value.
   * @param expandEdgeValidToCondition     conditions for successive edges' valid_to values. {@code null}
   *                                       to impose no restriction on the value.
  ...
  public ExpansionCriteria() {
    this(x -> true, x-> true, x->true, x->true, x->true, x->true, x->true, x->true, x->true, x->true);
  }
  ...
  public boolean checkExpansion(ExpandEmbeddingTPGM path, TemporalEdgeWithTiePoint edge) {
    ...
    return checkLifetimeConditions(txLifetime, validLifetime);
    // all conditions met
  }
  ...
  private boolean checkVertexConditions(Long[] sourceTimeData, Long[] targetTimeData) {
    ...
    return expandVertexValidToCondition == null || expandVertexValidToCondition.apply(
  ...
  private boolean checkEdgeConditions(Long[] e1TimeData, Long[] e2TimeData) {
    ...
    return expandEdgeValidToCondition == null || expandEdgeValidToCondition.apply(
  ...
  private boolean checkLifetimeConditions(Long txLifetime, Long validLifetime) {
    ...
    return validLifetimeCondition == null || validLifetimeCondition.apply(validLifetime);
  }
