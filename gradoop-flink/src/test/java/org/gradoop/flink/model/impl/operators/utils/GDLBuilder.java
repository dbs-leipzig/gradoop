...
public class GDLBuilder {
  ...
  public static class GraphWithinDatabase implements IWithDependencies {
    ...
    GraphWithinDatabase addClosedPattern(Collection<String> references,
    ...
    public String toString(boolean withVariableName) {
      return tp.toString() + (sb == null ? "[]" :
        "[" + sb.stream().map(x -> x.toString(withVariableName)).collect(Collectors.joining(" ")) +
          "]");
    }
    ...
    public String compileWith(HashMap<String, IWithDependencies> infoToExpand,
      ...
      return tp.toString() + (sb == null ? "[]" : "[" +
        sb.stream().map(x -> x.compile(infoToExpand, withVariableName))
  ...
  public static class TupleBuilderWithDependencies<P> extends TupleBuilder<P> implements
    IWithDependencies {
  ...
  public static class PatternBuilder {
    ...
    public String compile(HashMap<String, IWithDependencies> toAssociate,
      boolean withVariableName) {
      if (src == null && dst == null) {
        return "";
      } else if (src == null || dst == null) {
        return src != null ?
          src.compileWith(toAssociate, withVariableName).replace("((", "(").replace("))", ")") :
          dst.compileWith(toAssociate, withVariableName).replace("((", "(").replace("))", ")");
      } else {
        StringBuilder sb = new StringBuilder();
        sb.append(
          src.compileWith(toAssociate, withVariableName).replace("((", "(").replace("))", ")"));
        ...
        if (e != null) {
          sb.append(
            e.compileWith(toAssociate, withVariableName).replace("[(", "[").replace(")]", "]"));
        }
        sb.append("->");
        return sb.append(
          dst.compileWith(toAssociate, withVariableName).replace("((", "(").replace("))", ")"))
  ...
  public static class TupleBuilder<P> {
    ...
    public static <K extends TupleBuilder, P> K generateWithVariableAndType(P parent, K tb,
      String v, String k) {
      tb.variableName = v;
      tb.typeName = ":" + k;
      tb.propbuilder = null;
  ...
  public static class PropList<P> {
    ...
    @Override
    public String toString() {
      return attrTo.entrySet().stream().map(x -> (x.getKey() + " : " +
        (x.getValue().equals("NULL") ? "NULL" : "\"" + x.getValue() + "\"")))
        .collect(Collectors.joining(",", " {", "}"));
    }
