...
public abstract class AbstractTestBuilder extends GradoopFlinkTestBase {
  ...
  protected void addToGraphAttribute(String variable, String type) {
    ...
    GDLBuilder.VertexBuilder.generateWithVariableAndType(null, aGV, belongToGraph(variable), type)
      .t();
  ...
  public static GDLBuilder.VertexBuilder<?> simpleVertex(String var, String type) {
    ...
    GDLBuilder.VertexBuilder.generateWithVariableAndType(null, a, var, type).propList()
      .put(var + "value", var + "type").plEnd();
    return a;
  }
  ...
  public static GDLBuilder.EdgeBuilder<?> simpleEdge(String var, String type) {
    ...
    GDLBuilder.EdgeBuilder.generateWithVariableAndType(null, a, var, type).propList()
      .put(var + "value", var + "type").plEnd();
    return a;
  }
  ...
  public String javify(String toJava, boolean withVariables) {
    try {
      ...
      return "\"" +
        toAssociate.get(toJava).compileWith(toAssociate, withVariables).replaceAll("\"", "\\\\\"")
          .replaceAll("\n", "\"+\n\t\t\t\"") + "\"";
    } catch (Exception e) {
  ...
  public void generateToFile(File f, String binaryClassName, String testPatterns, String left,
    String right) {
    try (PrintWriter out = new PrintWriter(f)) {
      out.println(packageString + "\n\nimport org.gradoop.flink.model.GradoopFlinkTestBase;\n" +
        ...
        Pattern.compile("\n", Pattern.LITERAL).splitAsStream(testPatterns.trim())
          ...
          Set<String> toJavify = new LinkedHashSet<>();
                        /*toAssociate.entrySet().stream()
                                .filter(y -> y.getKey().equals(x[0]) || y.getKey().equals(x[1])
                                || y.getKey().equals(x[2]))
          ...
          //Assert.assertTrue("they do not match", FusionTestUtils.graphEquals(output,loader
            // .getLogicalGraphByVariable("ab_edgeWithBeta_loop")));
          StringBuilder append =
            sb.append("\t@Test\n\tpublic void ").append(x[0]).append("_").append(x[1])
              .append("_to_").append(x[2]).append("() throws Exception {\n\t\t")
              .append("FlinkAsciiGraphLoader loader = getLoaderFromString(\n")
              .append(toJavify.stream().collect(Collectors.joining("+\n\t\t\t"))).append(");\n\t\t")
              .append("LogicalGraph " + left + " = loader.getLogicalGraphByVariable(\"")
              .append(x[0]).append("\");\n\t\t")
              .append("LogicalGraph " + right + " = loader.getLogicalGraphByVariable(\"")
              .append(x[1]).append("\");\n\t\t")
              .append(binaryClassName + " f = new " + binaryClassName + "();\n\t\t")
              .append("LogicalGraph output = f.execute(" + left + "," + right + ");\n\t\t").append(
              "Assert.assertTrue(\"no match provided\", FusionTestUtils.graphEquals(output,loader" +
                ".getLogicalGraphByVariable(\"")
              .append(x[2]).append("\")));\n\t}");
          return sb.toString();
  ...
  public void check() {
    getTestGraphLoader();
    System.out.println(
      "Current keys are: " + toAssociate.keySet().stream().collect(Collectors.joining(",")));
    loader = getLoaderFromString(
      toAssociate.values().stream().filter(x -> x instanceof GDLBuilder.GraphWithinDatabase)
        .map(x -> x.compileWith(toAssociate, true)).collect(Collectors.joining("\n")));
  }
