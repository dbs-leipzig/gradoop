...
public class ProductGenerator extends AbstractMasterDataGenerator {
  ...
  public DataSet<Vertex> generate() {
    List<MasterDataSeed> seeds = getMasterDataSeeds(FoodBrokerConstants.PRODUCT_VERTEX_LABEL);
    ...
    for (String name : fruits) {
      nameGroupPairs.add(new Tuple2<>(name, FoodBrokerConstants.PRODUCT_TYPE_FRUITS));
    }
    for (String name : vegetables) {
      nameGroupPairs.add(new Tuple2<>(name, FoodBrokerConstants.PRODUCT_TYPE_VEGETABLES));
    }
    for (String name : nuts) {
      nameGroupPairs.add(new Tuple2<>(name, FoodBrokerConstants.PRODUCT_TYPE_NUTS));
    }

    return env.fromCollection(seeds)
      .map(new Product(vertexFactory, foodBrokerConfig))
      .withBroadcastSet(env.fromCollection(nameGroupPairs), FoodBrokerConstants.NAMES_GROUPS_BC)
      .withBroadcastSet(env.fromCollection(adjectives), FoodBrokerConstants.ADJECTIVES_BC)
