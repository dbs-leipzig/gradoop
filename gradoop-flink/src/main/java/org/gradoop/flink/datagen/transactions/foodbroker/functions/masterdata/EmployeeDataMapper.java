...
public class EmployeeDataMapper implements MapFunction<Vertex, Tuple2<GradoopId, EmployeeData>> {
  ...
  public Tuple2<GradoopId, EmployeeData> map(Vertex v) throws Exception {
    reuseEmployeeData.setQuality(v.getPropertyValue(FoodBrokerConstants.QUALITY_KEY).getFloat());
    reuseEmployeeData.setCity(v.getPropertyValue(FoodBrokerConstants.CITY_KEY).getString());
