...
public class DrillUp extends Drill {
   ...
   * @param function        drill function which shall be applied to a property of a vertex
   * @param function        drill function which shall be applied to a property of an edge
   * @param function      	drill function which shall be applied to a property of a graph head
   * @param newPropertyKey	new property key
   * @param element					Element to be covered by the operation
   */
  DrillUp(String label, String propertyKey, DrillFunction vertexDrillFunction, DrillFunction edgeDrillFunction, DrillFunction graphheadDrillFunction, String newPropertyKey, Element element) {
    super(label, propertyKey, vertexDrillFunction, edgeDrillFunction, graphheadDrillFunction, newPropertyKey, element);
  }
  ...
  public LogicalGraph execute(LogicalGraph graph) {
    if (getElement() == Element.VERTICES) {
      ...
      graph = graph.transformEdges(
        ...
        new DrillUpTransformation<GraphHead>(getLabel(), getPropertyKey(), getGraphheadDrillFunction(),
          getNewPropertyKey(), drillAllLabels(), keepCurrentPropertyKey()));
    }
    return graph;
  }
