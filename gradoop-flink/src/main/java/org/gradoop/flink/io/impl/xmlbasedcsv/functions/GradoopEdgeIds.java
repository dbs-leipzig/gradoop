...
public class GradoopEdgeIds extends RichMapFunction<Edge, Edge> {
  ...
  public void open(Configuration parameters) throws Exception {
    ...
    map = getRuntimeContext().<HashMap<String, GradoopId>>getBroadcastVariable(
        XMLBasedCSVConstants.BROADCAST_ID_MAP).get(0);
  }

  @Override
  public Edge map(Edge edge) throws Exception {
    Properties properties = edge.getProperties();
    edge.setSourceId(map.get(properties.get(XMLBasedCSVConstants.PROPERTY_KEY_SOURCE).getString()));
    edge.setTargetId(map.get(properties.get(XMLBasedCSVConstants.PROPERTY_KEY_TARGET).getString()));
    return edge;
  }
}
