...
public class CreateEdges {
	...
	public static DataSet<Edge> create(GradoopFlinkConfig config, RdbmsConfig rdbmsConfig,
			ArrayList<TableToEdge> tablesToEdges, DataSet<Vertex> vertices, String limit) {

		DataSet<Edge> edges = null;

		int downLimit = Integer.parseInt(limit.split(",")[0]);
		int upLimit = Integer.parseInt(limit.split(",")[1]);
		System.out.println(downLimit + " " + upLimit);

		for (int i = downLimit; i < upLimit; i++) {
			TableToEdge table = tablesToEdges.get(i);
			...
			// converts table tuples (n:m relations)
				...
				// properties
				DataSet<Tuple3<String, String, Properties>> fkPropsTable = dsSQLResult.map(new FKandProps(i))
