package org.gradoop.flink.io.impl.rdbms.tempGraphDSUsing;

import java.util.List;

import org.apache.flink.api.common.functions.MapFunction;
import org.apache.flink.api.common.functions.RichMapFunction;
import org.apache.flink.configuration.Configuration;
import org.apache.flink.types.Row;
import org.gradoop.common.model.impl.properties.Properties;
import org.gradoop.flink.io.impl.graph.tuples.ImportEdge;
import org.gradoop.flink.io.impl.rdbms.constants.RDBMSConstants;
import org.gradoop.flink.io.impl.rdbms.tuples.PKTuple;

public class FkToEdge extends RichMapFunction<Row,ImportEdge<String>> {
	private int tablePos;
	private List<TablesToEdges> tables;
	private TablesToEdges currentTable;
	
	public FkToEdge(int tablePos){
		this.tablePos = tablePos;
	}

	@Override
	public ImportEdge<String> map(Row tuple) throws Exception {
		currentTable = tables.get(tablePos);
		String pkConcat = concatPK(tuple);
		ImportEdge<String> e = new ImportEdge<String>();
		e.setLabel(currentTable.getEndAttribute().getName());
		e.setSourceId(pkConcat);
		e.setTargetId(tuple.getField(tuple.getArity()-1).toString());
		e.setProperties(new Properties());
		return e;
	}
	
	public String concatPK(Row tuple) {
		String pkConcat = "";
		if (currentTable.getPrimaryKeys().size() > 1) {
			for (int i = 0; i < tuple.getArity()-1; i++) {
				pkConcat += tuple.getField(i) + RDBMSConstants.PK_DELIMITER;
			}
		} else {
			pkConcat = tuple.getField(0).toString();
		}
		return pkConcat;
	}
	
	public void open(Configuration parameters) throws Exception {
		this.tables = getRuntimeContext().getBroadcastVariable("tables");
	}
}
