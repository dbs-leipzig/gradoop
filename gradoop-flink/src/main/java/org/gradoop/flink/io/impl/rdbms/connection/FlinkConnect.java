	...
	public static DataSet<Row> connect(ExecutionEnvironment env, RdbmsConfig rdbmsConfig, int rowCount, String sqlQuery,
		...
		try {
			jdbcInput = JDBCInputFormat.buildJDBCInputFormat()
					...
					.setQuery(sqlQuery + PageinationQueryChooser.choose(rdbmsConfig.getRdbms()))
					.setRowTypeInfo(typeInfo)
					.setParametersProvider(
							new GenericParameterValuesProvider(ParametersChooser.choose(rdbmsConfig.getRdbms(),parallelism, rowCount)))
					.finish();
		} catch (Exception e) {
