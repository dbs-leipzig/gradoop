...
public class ExpandResult
  implements FlatMapFunction<GraphEmbeddingsPair, int[]> {

  @Override
  public void flatMap(GraphEmbeddingsPair graphEmbeddingsPair,
    Collector<int[]> collector) throws Exception {

    for (int i = 0; i < graphEmbeddingsPair.getPatternEmbeddings().getPatternCount(); i++) {
      collector.collect(graphEmbeddingsPair.getPatternEmbeddings().getPattern(i));
    }
  }
}
