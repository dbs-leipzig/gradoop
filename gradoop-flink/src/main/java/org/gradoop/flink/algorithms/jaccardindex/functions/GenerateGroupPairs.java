...
public class GenerateGroupPairs implements
  ...
  public GenerateGroupPairs(int groupSize, NeighborhoodType neighborhoodType) {
  ...
  public void reduce(Iterable<Tuple4<IntValue, GradoopId, GradoopId, IntValue>> values,
    ...
    for (Tuple4<IntValue, GradoopId, GradoopId, IntValue> edge : values) {
      for (int i = 0; i < visitedCount; i++) {
        ...
        prior.f1 = neighborhoodType.equals(NeighborhoodType.OUTDEGREE) ? edge.f1 : edge.f2;

        int oldValue = prior.f2.getValue();

        // TODO: statt sum kann hier max rein (Denominator Teil1)
      ...
      if (visitedCount < groupSize) {
        if (!initialized) {
          ...
          for (int i = 0; i < groupSize; i++) {
            ...
            tuple.f0 = neighborhoodType.equals(NeighborhoodType.OUTDEGREE) ? edge.f1.copy() : edge.f2.copy();
