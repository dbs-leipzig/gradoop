 ...
 * 3. run main method
 */
public class FrequentLossPatternsWithOutput
  ...
  public static void main(String[] args) throws Exception {
    ...
    String graphHeadPath = FrequentLossPatternsWithOutput.class
      .getResource("/data/json/foodbroker/graphs.json").getFile();

    String vertexPath = FrequentLossPatternsWithOutput.class.
      getResource("/data/json/foodbroker/nodes.json").getFile();

    String edgePath = FrequentLossPatternsWithOutput.class
    ...
    btgs = btgs.apply(new ApplyTransformation(
      ...
      FrequentLossPatternsWithOutput::relabelVerticesAndRemoveProperties,
      FrequentLossPatternsWithOutput::dropEdgeProperties
    ));
    ...
    frequentSubgraphs = frequentSubgraphs.apply(new ApplyTransformation(
      FrequentLossPatternsWithOutput::addSupportToGraphHead,
      TransformationFunction.keep(),
      TransformationFunction.keep()
    ));
    ...
    String outPath =
      System.getProperty("user.home") + "/lossPatterns_" + Time.now();

    new DOTDataSink(outPath + ".dot", true).write(frequentSubgraphs);
  ...
  public String getDescription() {
    return  FrequentLossPatternsWithOutput.class.getName();
  }
}
