...
public class LogicalGraphToNestedModel extends AbstractRunner {
   ...
   *
   */
  private DataSet<ImportEdge<String>> onlyEdges;
  ...
  public LogicalGraphToNestedModel(String pathToDataLake) {
    ...
    onlyEdges = getExecutionEnvironment().readFile(pif,pathToDataLake)
        .flatMap(new TripleSplit());
    allEdges = onlyEdges.crossWithTiny(head);
  ...
  public void addFile(String edgePath, String vertexPath) {
    // Heads to be assoicated to the file
    ...
    DataSet<ImportEdge<String>> rawEdges =
      getExecutionEnvironment().readFile(pif,edgePath)
        .flatMap(new TripleSplit());

    onlyEdges
      .union(rawEdges)
      .distinct(new ImportEdgeId<>());

    DataSet<Tuple2<ImportEdge<String>, GradoopId>> rawEdges2 = rawEdges
      .crossWithTiny(head);

    allEdges.union(rawEdges.crossWithTiny(head));

    DataSet<Tuple2<ImportVertex<String>,GradoopId>> rawVertices = rawEdges2
      .flatMap(new ExtractVerticesFromEdges<>())
      .distinct(new ImportVertexId<>());

    DataSet<Tuple2<ImportVertex<String>,GradoopId>> rawVerticesFromFile = getExecutionEnvironment()
      .readFile(pif,vertexPath)
      .map(new ImportVertexId<>())
      .cross(head);

    rawVertices = rawVertices.union(rawVerticesFromFile)
      .distinct(new ImportVertexId<>());
  ...
  public GraphCollection collectSourcesAsCollection() {
    ...
    DataSet<Tuple2<String,Edge>> epgmEdges = onlyEdges
      .join(vertexIdPair)
      .where(1).equalTo(0)
  ...
  public static void main(String[] args) throws Exception {
    ...
    LogicalGraphToNestedModel model = new LogicalGraphToNestedModel(inputPath);
    File folder = new File(subgraphs);
    Map<String, List<String>> l = Arrays
    ...
    for (List<String> files : l.values()) {
      if (files.size()==2) {
        ...
        if (edgeFile != null && vertexFile != null) {
          model.addFile(edgeFile,vertexFile);
        }
      }
    }
