...
public class VertexIndexTest extends AccumuloStoreTestBase {

  private static final String TEST01 = "vertex_index_test_01";
  private static final String TEST02 = "vertex_index_test_02";

  /**
   * Do insert by store, and query all vertices that fulfill filter predicate
   *
  ...
  public void test01_doInsertAndQueryWithPredicate() throws Throwable {
    storeInsertAndTest(TEST01, (loader, store, config) -> {
      AccumuloElementFilter<Vertex> filter = AccumuloFilters.labelIn("Person");

      List<Vertex> graphs = new ArrayList<>(loader.getVertices())
        .stream()
        .filter(filter)
        .collect(Collectors.toList());

      AccumuloIndexedDataSource dataSource = new AccumuloIndexedDataSource(config, store);
      List<Vertex> query = dataSource.getVertices(filter)
        .collect();
   ...
   * query Vertexs within id seeds that fulfill filter predicate
   *
   * @throws Throwable if test error
   */
  @Test
  public void test02_doInsertAndQueryWithIds() throws Throwable {
    storeInsertAndTest(TEST02, (loader, store, config) -> {
      AccumuloElementFilter<Vertex> filter = AccumuloFilters.labelIn("Person");
      for (int i = 0; i < 10; i++) {
        List<Vertex> graphs = sample(new ArrayList<>(loader.getVertices())
          .stream()
          .filter(filter)
        ...
        AccumuloIndexedDataSource dataSource = new AccumuloIndexedDataSource(config, store);
        List<Vertex> query = dataSource
          .getVertices(getExecutionEnvironment().fromCollection(ids), filter)
          .collect();
