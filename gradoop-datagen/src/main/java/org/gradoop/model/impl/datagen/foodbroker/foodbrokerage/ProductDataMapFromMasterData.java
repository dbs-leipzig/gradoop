 ...
 */
public class ProductDataMapFromMasterData
  implements GroupReduceFunction<AbstractMasterDataTuple,
  Map<GradoopId, ProductTuple>> {

  @Override
  public void reduce(Iterable<AbstractMasterDataTuple> iterable,
    Collector<Map<GradoopId, ProductTuple>> collector) throws
    Exception {
    Map<GradoopId, ProductTuple> map = Maps.newHashMap();
    for(AbstractMasterDataTuple tuple : iterable) {
      map.put(tuple.getId(), (ProductTuple) tuple);
    }
