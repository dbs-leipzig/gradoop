 ...
 */
package org.gradoop.dataintegration.importer.impl.rdbms.metadata;
...
public abstract class TableToElement {
  ...
  /**
   * Row header for row data representation of relational data.
   */
  ...
  TableToElement(
    ...
    this.rowheader = Lists.newArrayList();
  ...
  /**
   * Get just row header tuples of foreign key attributes.
   *
   * @return list of row header tuples of foreign key attributes
   */
  public ArrayList<RowHeaderTuple> getForeignKeyHeader() {
    ArrayList<RowHeaderTuple> foreignKeyHeader = Lists.newArrayList();

    for (RowHeaderTuple rowHeaderTuple : this.rowheader) {
      if (rowHeaderTuple.getAttributeRole().equals(FK_FIELD)) {
        foreignKeyHeader.add(rowHeaderTuple);
      }
    }
   ...
   */
  void setRowheader(ArrayList<RowHeaderTuple> rowheader) {
